...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsedPrice = new Prisma.Decimal(price); // ✅ Fix: Use `Prisma.Decimal`
    ...
    // ✅ Fetch existing product to keep image data if not updating
    ...
    const updatedProduct = await prisma.product.update({
      ...
      data: {
        ...
        stock: parsedStock,
        description,
        imageUrl: imageUrl || existingProduct.imageUrl, // ✅ Keep existing if not updated
        imageData: imageData || existingProduct.imageData, // ✅ Keep existing if not updated
      },
    });
